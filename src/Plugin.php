...
class Plugin
{
    ...
    public function __construct()
    {
        ...
        add_action('init', array($this, 'checkForUpdates'));
        ...
        add_action('init', function () {
            ...
            foreach (self::$actions as $action) {
                add_action($action['hook'], $action['class'], $action['priority'], $action['args']);
            }
        });
    }
    ...
    {
        foreach ($this->autoload as $load) {
            new $load();
        }

        new \Kylwes\Ignition\User();
    }
